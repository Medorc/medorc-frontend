  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition cursor-pointer hover:bg-gray-50">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
        ...
        {/* This button will now correctly navigate to the view page */}
        <button 
          onClick={() => navigate(`/recordview/${record.record_id}`)}
          className="px-4 py-2 text-sm font-medium border rounded-lg hover:bg-gray-50 cursor-pointer"
        >
          View Details
        </button>
      </div>
          ...
          {tags.map((tag, i) => (
            <span
              ...
              className={`px-3 py-1 text-xs rounded-full font-medium ${
                tag === "Hospitalization"
                  ? "bg-orange-400/10 text-orange-600 border border-orange-200"
                  : "bg-purple-400/10 text-purple-600 border border-purple-200"
              }`}
            >
              {tag}
            </span>
