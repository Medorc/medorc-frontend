...
function CreateRecordPage() {
  ...
  const [formData, setFormData] = useState({
    basicDetails: {
      ...
      treatment_undergone: "",
      // reg_no is added dynamically by the form input
    },
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.documents.prescriptions && !formData.documents.lab_results) {
      toast.info("Please upload at least one document.");
      // You can uncomment the return below if you want to enforce this check
      // return; 
    }
    ...
    const dataToSend = {
      ...
      // --- FIX: Map 'prescriptions' (plural) to 'prescription' (singular) for backend ---
      documents: {
    ...
    try {
      // --- FIX: Removed the line that was deleting reg_no ---
