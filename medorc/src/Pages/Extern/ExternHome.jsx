...
export default function ExternHome() {
  ...
  const handleScan = async (detectedCodes) => {
    ...
    try {
      ...
      if (visibility) {
        navigate(`/extern/patientrecords?qr_code=${code}`);
      } else {
        navigate(`/extern/PatientBasicDetails?qr_code=${code}`);
      }
    } catch (error) {
      console.error("Error fetching user profile:", error);
    } finally {
  ...
  const handleSearch =async () => {
    ...
    try{
      ...
      if (visibility) {
        navigate(`/extern/patientrecords?shc_code=${scanResult}`);
      } else {
        navigate(`/extern/PatientBasicDetails?shc_code=${scanResult}`);
      }
    }catch(error){
      console.error("Error fetching user profile:", error);
    }finally{
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="mt-4">
                    <button
                      ...
                      onClick={() => navigate("/extern/profile")}
                    >
                      View Profile
                    </button>
                  </div>
                ...
                <div className="self-start md:self-center">
                  <div className="w-24 h-24 bg-gray-300 rounded-full">
                    <img src={user.photo||"image.png"} className="w-full h-full object-cover" alt="profile" />
                  </div>
                </div>
              </div>
            </div>
